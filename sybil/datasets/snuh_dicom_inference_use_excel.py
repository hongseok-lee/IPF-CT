...
class Inference_Dataset(data.Dataset):
    def __init__(self, args, df):
        ...
        self.snu_label = pd.read_csv('/data1/IPF_CT/snuh_label.csv')
        self.args = args
    ...
    def __getitem__(self, index):
        ...
        _dirpath = (sample['dir_path'].split('IPF_CT/')[2]).replace('IPF_prog_predic//', 'IPF_prog_predic/')
        row = self.snu_label[self.snu_label.dir_path == _dirpath]
        
        assert len(row) == 1, [_dirpath, self.snu_label.dir_path]
        row = row.iloc[0]
        try:
            item = {}
            ...
            if 'plus' in self.args.model_type:
                ...
                clinical_dict['age_ct'] = torch.Tensor([norm_age_ct])
                if row['성별'] in ['male']:
                    clinical_dict['sex'] = torch.Tensor([1, 0])
                else:
                    clinical_dict['sex'] = torch.Tensor([0, 1])
                
                item['x'] = {'img': item['x'], 'clinical': clinical_dict}
                item['x']['puid'] = str(row.ID)
                item['x']['sex_str'] = '-1'
    ...
    def get_images(self, sample):
        ...
        resampled, zf = resize(dicom_pixels, dicom_data['pixel_spacing'], [1,1,1], 'nearest')
