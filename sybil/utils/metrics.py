...
def get_regression_metrics(logging_dict, args):
    stats_dict = OrderedDict()
    for v in args.pred_volume:
        # golds = np.concatenate([d[v].detach().cpu().numpy() for d in logging_dict["golds"]]).reshape(-1)
        # preds = np.concatenate([d[v].detach().cpu().numpy() for d in logging_dict["probs"]]).reshape(-1)
        golds = logging_dict['golds'][v].detach().cpu().numpy().reshape(-1)
        preds = logging_dict['probs'][v].detach().cpu().numpy().reshape(-1)

        stats_dict[f"{v}_mse"] = np.mean((golds - preds) ** 2)
        stats_dict[f"{v}_mae"] = np.mean(np.abs(golds - preds))

    return stats_dict

def get_classification_metrics(logging_dict, args):
    stats_dict = OrderedDict()
    if isinstance(logging_dict["golds"], dict):
        golds = np.array(logging_dict["golds"]["survival"]).reshape(-1)
    else:
        golds = np.array(logging_dict["golds"]).reshape(-1)
    if isinstance(logging_dict["probs"], dict):
        probs = np.array(logging_dict["probs"]["survival"])
    else:
...
def get_survival_metrics(logging_dict, args):
    ...
    if isinstance(logging_dict["golds"], dict):
        censor_times, probs, golds, time_at_event_f = (
            logging_dict["censors"],
            logging_dict["probs"]['survival'].detach().cpu().numpy(),
            logging_dict["golds"]['survival'].detach().cpu().numpy(),
            logging_dict["time_at_event_f"],
        )
    else:
        censor_times, probs, golds, time_at_event_f = (
            logging_dict["censors"],
    ...
    # [HS] C-index has error during its calculation. 
    # TODO: fix it. 
    ...
    # Prepare data for cumulative_dynamic_auc
    survival_train = args.survival_train
    survival_dev = np.array(list(zip(golds, time_at_event_f)), dtype=[('death', bool), ('futime', float)])

    # Calculate time-dependent AUROC
    time_points = np.arange(1, args.max_followup + 1)
    t_auc, mean_auc = cumulative_dynamic_auc(survival_train, survival_dev, probs, time_points)

    # Log time-dependent AUROC
    for i, t in enumerate(time_points):
        stats_dict[f"{t}_year_time_dependent_auroc"] = t_auc[i]
    stats_dict["mean_time_dependent_auroc"] = mean_auc
